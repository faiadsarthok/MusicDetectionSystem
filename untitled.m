...
% Tests the fingerprinting + matching pipeline using clean digital
% excerpts pulled directly from the processed database audio - no
% microphone involved. This confirms the algorithm itself is correct,
% independent of real-world recording conditions.
%
% Requires extractPeaks.m to be on the MATLAB path.

load('C:\CEP_Project\database\fingerprintDB.mat');  % loads fingerprintDB, songList

processedFolder = 'C:\CEP_Project\processed\';
files = dir(fullfile(processedFolder, '*.wav'));
targetFs = 8000;
clipDuration = 5; % seconds
...
for songID = 1:total
    ...
    % Pick a random 5-second chunk (skip first 10s to avoid intros/silence)
    maxStart = length(y) - clipDuration*Fs;
    
    % Ensure maxStart is valid for short songs
    if maxStart <= round(Fs*10)
        startSample = 1; 
    else
        startSample = randi([round(Fs*10), maxStart]);
    end
    
    clip = y(startSample : startSample + clipDuration*Fs - 1);
    
    % Normalize amplitude to match live GUI
    if max(abs(clip)) > 0
        clip = clip / max(abs(clip)) * 0.95;
    end

    % Match the exact preprocessing used in build_database.m / MusicIDApp.m
